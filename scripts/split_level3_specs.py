#!/usr/bin/env python3
"""
Split Level 3 detailed spec sections (appended to Levels/Level 3/CHECKLIST.md)
...
- Output path: Levels/Level 3/<num-padded>-<slug>/SPEC.md
...
def split_specs(src_path: Path, out_dir: Path) -> int:
    ...
    if not sections:
        print("No Level 3 project sections found (### <num>) <Title>).")
        return 0
...
def main(argv: list[str]) -> int:
    ...
    src_path = root / "Levels" / "Level 3" / "CHECKLIST.md"
    if not src_path.exists():
        print(f"Source file not found: {src_path}", file=sys.stderr)
        return 1
    out_dir = src_path.parent
    count = split_specs(src_path, out_dir)
    print(f"Wrote {count} Level 3 SPEC.md files under: {out_dir}")
    return 0
